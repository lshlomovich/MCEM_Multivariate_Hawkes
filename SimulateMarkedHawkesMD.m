...
% CITE: CODE TAKEN FROM 
% https://karwailim.github.io/
...
% In Proceedings of the 6th Asian Conference on Machine Learning (ACML), pp. 238-253. JMLR. 2016.
...
inv_mu    = 1 ./ mu;
% Addition by Leigh for Python usage:
if size(inv_mu) == [1,D]
    inv_mu = transpose(inv_mu);
end
...
for j = 2:N
    ...
    if saveX
        if Xconst
            % for Python:
            % disp(parX)
            % parX = transpose(parX); %ignore if not using Python
            % disp(m_star)
            % X_size(j) = parX(m_star,1);
            X_size(j) = parX(m_star);
        elseif Xgam
